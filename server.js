...
function groupNextKey(code, battleNo) { return `${String(code || '').toUpperCase()}|${Number(battleNo) || 0}`; }
function pruneGroupSignals() { const now = Date.now(); for (const [key, value] of groupNextSignals) { if (now - value.updatedAt > groupNextTtlMs) groupNextSignals.delete(key); } }
app.post('/api/rooms/group-sync-next', (req, res) => {
  ...
  current.players.add(playerId); current.updatedAt = Date.now(); if (current.players.size >= 2) current.ready = true;
...
app.use(async (req, res) => {
  try {
    ...
    const upstream = await fetch(target, { method: req.method, headers: { ...req.headers, host: new URL(TARGET).host }, body: ['GET', 'HEAD'].includes(req.method) ? undefined : JSON.stringify(req.body ?? {}), redirect: 'manual' });
    const contentType = upstream.headers.get('content-type') || '';
    const headers = {};
    upstream.headers.forEach((value, key) => { if (!['content-length', 'transfer-encoding', 'content-encoding', 'location'].includes(key.toLowerCase())) headers[key] = value; });
    if (contentType.includes('text/html')) {
      ...
      const groupPickVisibility = `<script>(function(){let roomCode='';const originalFetch=window.fetch.bind(window);window.fetch=async function(input,init){try{const url=typeof input==='string'?input:input?.url||'';if(url.includes('/api/rooms/group-roster')||url.includes('/api/rooms/group-pick')||url.includes('/api/rooms/group-confirm')){const body=typeof init?.body==='string'?JSON.parse(init.body):null;if(body?.code)roomCode=String(body.code).toUpperCase()}}catch(e){}return originalFetch(input,init)};const esc=s=>String(s??'').replace(/[&<>\\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\\"':'&quot;',"'":'&#39;'}[c]));const roleLine=(roles,role)=>{const name=Object.entries(roles||{}).find(([,v])=>v===role)?.[0];return name?'✅ '+role+'：'+esc(name):'⬜ '+role+'：選択待ち'};let lastSignature='';async function refresh(){if(!roomCode)return;const waiting=[...document.querySelectorAll('.waiting')].find(el=>((el.textContent||'').includes('相手の選択中')||(el.textContent||'').includes('選択を待っています')));if(!waiting)return;try{const r=await originalFetch('/api/rooms/'+encodeURIComponent(roomCode),{cache:'no-store'});if(!r.ok)return;const room=await r.json();const group=room.group;if(!group?.roles)return;const mineKey=room.host?.id===localStorage.getItem('kingdom-player-id')?'host':'guest';const enemyKey=mineKey==='host'?'guest':'host';const roles=['先鋒','副将','大将'];const signature=roles.map(role=>role+':'+(Object.entries(group.roles[enemyKey]||{}).find(([,v])=>v===role)?.[0]||'')).join('|');if(signature===lastSignature&&document.querySelector('#group-live-picks'))return;lastSignature=signature;let box=document.querySelector('#group-live-picks');if(!box){box=document.createElement('div');box.id='group-live-picks';box.style.cssText='margin:16px 0;padding:16px;border:1px solid #d5ad55;border-radius:14px;background:linear-gradient(135deg,#21160c,#0d0d0f);color:#fff0bd;text-align:left';waiting.parentElement?.insertBefore(box,waiting)}box.innerHTML='<strong style="display:block;text-align:center;margin-bottom:10px">相手軍の現在編成</strong>'+roles.map(role=>'<div style="padding:5px 0">'+roleLine(group.roles[enemyKey],role)+'</div>').join('')}catch(e){}}new MutationObserver(()=>refresh()).observe(document.body,{childList:true,subtree:true});setInterval(refresh,500);refresh()})();</script>`;
      const groupPickSync = `<script>(function(){let roomCode='';const baseFetch=window.fetch.bind(window);window.fetch=async function(input,init){try{const url=typeof input==='string'?input:input?.url||'';if(url.includes('/api/rooms/group-roster')||url.includes('/api/rooms/group-pick')||url.includes('/api/rooms/group-confirm')){const body=typeof init?.body==='string'?JSON.parse(init.body):null;if(body?.code)roomCode=String(body.code).toUpperCase()}}catch(e){}return baseFetch(input,init)};let lastTurn=-1;let lastPhase='';let lastRole='';const check=async()=>{if(!roomCode)return;const waiting=[...document.querySelectorAll('.waiting')].find(el=>((el.textContent||'').includes('相手の選択中')||(el.textContent||'').includes('選択を待っています')));if(!waiting)return;try{const r=await baseFetch('/api/rooms/'+encodeURIComponent(roomCode),{cache:'no-store'});if(!r.ok)return;const room=await r.json();const group=room.group;if(!group)return;const id=localStorage.getItem('kingdom-player-id')||'';const picker=group.turn<=5?[room.host?.id,room.guest?.id,room.guest?.id,room.host?.id,room.host?.id,room.guest?.id][Number(group.turn)]:null;const changed=Number(group.turn)!==lastTurn||group.phase!==lastPhase||picker!==lastRole;if(changed){lastTurn=Number(group.turn);lastPhase=String(group.phase||'');lastRole=String(picker||'');if((group.phase==='pick'&&picker===id)||group.phase==='confirm'||group.phase==='battle')window.location.reload();}}catch(e){}};check();setInterval(check,500)})();</script>`;
      html = html.replace('</head>', hideGreatGeneralTrial + groupSyncGate + groupPickVisibility + groupPickSync + '</head>');
      res.status(upstream.status).set(headers).send(html); return;
    }
    const buffer = Buffer.from(await upstream.arrayBuffer());
    res.status(upstream.status).set(headers).send(buffer);
  } catch (error) { console.error('proxy error', error); res.status(502).send('KINGDOM upstream unavailable'); }
});
...
server.on('upgrade', (req, socket, head) => {
  ...
  wss.handleUpgrade(req, socket, head, client => {
    ...
    client.on('close', closeBoth); upstream.on('close', closeBoth); upstream.on('error', closeBoth); client.on('error', closeBoth);
  });
});
