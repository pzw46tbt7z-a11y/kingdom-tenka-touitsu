...
app.use(async (req, res) => {
  try {
    ...
    if (contentType.includes('text/html')) {
      ...
      const groupPickSync = `<script>(function(){let roomCode='';const baseFetch=window.fetch.bind(window);window.fetch=async function(input,init){try{const url=typeof input==='string'?input:input?.url||'';if(url.includes('/api/rooms/group-roster')||url.includes('/api/rooms/group-pick')){const body=typeof init?.body==='string'?JSON.parse(init.body):null;if(body?.code)roomCode=String(body.code).toUpperCase()}}catch(e){}return baseFetch(input,init)};const NativeWebSocket=window.WebSocket;const sockets=new Set();window.WebSocket=new Proxy(NativeWebSocket,{construct(target,args){const ws=new target(...args);sockets.add(ws);ws.addEventListener('close',()=>sockets.delete(ws));return ws;}});let lastTurn=-1;setInterval(async()=>{const waiting=[...document.querySelectorAll('.content.center')].find(el=>(el.textContent||'').includes('第4選択を待っています'));if(!waiting||!roomCode)return;try{const r=await baseFetch('/api/rooms/'+encodeURIComponent(roomCode),{cache:'no-store'});if(!r.ok)return;const room=await r.json();const group=room.group;if(!group||group.phase!=='pick'||group.turn<=3||group.turn===lastTurn)return;lastTurn=group.turn;const payload=JSON.stringify({type:'kingdom.room.update',payload:{room:room.code,data:room}});sockets.forEach(ws=>{try{if(ws.readyState===1&&typeof ws.onmessage==='function')ws.onmessage(new MessageEvent('message',{data:payload}))}catch(e){}})}catch(e){}},400)})();</script>`;
